####SHOT CHARTS FROM OWEN####


#Load packages and create a  percent formatter function
...
players = tibble::as_tibble(data.frame(players_data$resultSets$rowSet[[1]], stringsAsFactors = FALSE))
...
p <- plot_court(court_themes$dark) +
  geom_polygon(
    data = df,
    aes(
      x = adj_x,
...
plot_court(court_theme = court_theme$light, use_short_three = F)
